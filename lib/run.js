...
async function run() {
    ...
    try {
        let { exitCode, stdout, stderr, parsedOut } = await cljKondo(cljKondoArgs);
        ...
        if (parsedOut) {
            ...
            if(conclusion !== 'success'){
                console.log(`clj-kondo detected some problems and exited with code (${exitCode}). Please check found problems at https://github.com/${GITHUB_REPOSITORY}/runs/${id}`);
            }
            await updateCheck(id, conclusion, {
                title: checkName,
                summary: checkSummary
            });
        } else {
